...
class TestMarketComparison(unittest.TestCase):
    ...
    def test_beta(self, mock_read_csv, mock_get_spy_benchmark, mock_benchmark_variance):
        # Mock portfolio data
        mock_portfolio_returns = pd.DataFrame(
            {"pct_change": [0.01, 0.02, -0.01, 0.03, 0.02]}
        )  # Portfolio returns
        mock_benchmark_returns = pd.DataFrame(
            {"pct_change": [0.005, 0.01, -0.005, 0.015, 0.01]}
        )  # Benchmark returns
        # fake csv read
        mock_read_csv.side_effect = [mock_portfolio_returns, mock_benchmark_returns]

        # Calculate expected beta manually
        covariance = mock_portfolio_returns["pct_change"].cov(
            mock_benchmark_returns["pct_change"]
        )
    ...
    def test_alpha(
        self, mock_beta, mock_benchmark_avg_return, mock_annualized_avg_return
    ):
    ...
    def test_risk_adjusted_return(
        self, mock_risk_premium, mock_benchmark_vol, mock_portfolio_vol
    ):

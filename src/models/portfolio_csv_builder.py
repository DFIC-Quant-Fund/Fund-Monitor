...
end_date = (pd.Timestamp.now() + timedelta(days=1)).strftime(
    "%Y-%m-%d"
)  # yfinance end_dates are exclusive for download and history functions
# end_date = '2025-05-30'
...
class Portfolio:
    def __init__(self, start_date, end_date, STARTING_CASH, folder_prefix):
        ...
        self.securities: Dict[
            str, Security
        ] = {}  # dictionary of ticker -> Security objects
    ...
    def create_table_exchange_rates(self):
        ...
        exchange_rates = {
            ...
            "USD": yf.Ticker("CAD=X").history(start=self.start_date, end=self.end_date)[
                "Close"
            ],
        }
        ...
        pd.DataFrame(self.exchange_rates).to_csv(
            os.path.join(self.output_folder, exchange_rates_file), index_label="Date"
        )
        ...
        if os.path.exists(conversions_path):
            ...
            if not df.empty:
                ...
                self.conversions = pd.DataFrame(
                    columns=["Currency_From", "Currency_To", "Amount", "Rate"]
                )
        else:
    ...
    def create_table_prices(self):
        ...
        for ticker in self.tickers:
            ...
            if prices.empty:
                ...
                for variant in ticker_variants:
                    prices = yf.Ticker(variant).history(
                        start=self.start_date,
                        end=self.end_date,
                        actions=True,
                        auto_adjust=False,
        ...
        pd.DataFrame(self.prices).to_csv(
            os.path.join(self.output_folder, prices_file), index_label="Date"
        )
    ...
    def _calculate_original_weights(self):
        ...
        for _, row in initial_trades.iterrows():
            ...
            value_cad = self._calculate_ticker_value_in_cad(
                ticker, quantity, price, first_trade_date
            )
            initial_values[ticker] = value_cad
            total_value_cad += value_cad

        weights = {
            ticker: value / total_value_cad for ticker, value in initial_values.items()
        }
    ...
    def _simulate_holdings_to_date(self, target_date, additional_trades=None):
        ...
        if additional_trades is not None and not additional_trades.empty:
            additional_up_to_date = additional_trades[
                additional_trades.index <= target_date
            ]
            trades_up_to_date = pd.concat(
                [trades_up_to_date, additional_up_to_date]
            ).sort_index()
    ...
    def _add_quarterly_rebalancing_trades(self):
        ...
        logger.info(
            f"Adding quarterly rebalancing trades for {len(quarter_ends)} quarter-ends"
        )
        ...
        for qe_date in quarter_ends:
            ...
            portfolio_value_cad = self._calculate_portfolio_value_at_date(
                qe_date, current_holdings
            )

            if portfolio_value_cad == 0:
                logger.warning(
                    f"Skipping rebalancing on {qe_date}: portfolio value is zero"
                )
                continue
            ...
            if rebalancing_trades:
                ...
                current_qe_trades = [
                    t for t in rebalancing_trades if t["Date"] == qe_date
                ]
        ...
        if rebalancing_trades:
            ...
            logger.info(
                f"Added {len(rebalancing_trades)} rebalancing trades to benchmark portfolio"
            )
        ...
        for ticker in self.tickers:
            try:
                ...
                if splits_series is None or len(splits_series) == 0:
                    split_events[ticker] = {}
                    continue
                splits_series.index = pd.to_datetime(splits_series.index).tz_localize(
                    None
                )
                splits_series = splits_series.loc[self.start_date : self.end_date]
                splits_series = splits_series[splits_series != 0]
                split_events[ticker] = {
                    idx: float(val) for idx, val in splits_series.items()
                }
            except Exception as e:
    ...
    def create_table_daily_holdings(self):
        ...
        for i, date in enumerate(self.valid_dates):
            ...
            if date in self.trades.index:
                ...
                for _, row in rows_for_date.iterrows():
                    quantity = row["Quantity"]
                    ticker = row["Ticker"]
                    self.holdings.at[date, ticker] = (
                        self.holdings.loc[date, ticker] + quantity
                    )
        ...
        pd.DataFrame(self.holdings).to_csv(
            os.path.join(self.output_folder, holdings_file), index_label="Date"
        )
    ...
    def create_table_cash(self):
        ...
        for i, date in enumerate(self.valid_dates):
            ...
            if (
                self.conversions is not None
                and not self.conversions.empty
                and date in self.conversions.index
            ):
                ...
                for _, row in rows_for_date.iterrows():
                    ...
                    rate = (
                        float(row["Rate"])
                        if not (
                            isinstance(row["Rate"], float) and math.isnan(row["Rate"])
                        )
                        else None
                    )
                    ...
                    if rate is None:
                        if date not in self.exchange_rates.index:
                            ...
                            try:
                                idx = self.exchange_rates.index.get_indexer(
                                    [date], method="pad"
                                )[0]
                                if idx == -1:
                                    raise ValueError(
                                        f"No exchange rate found on or before {date}"
                                    )
                                current_usd_rate = float(
                                    self.exchange_rates.iloc[idx]["USD"]
                                )
                            except Exception as e:
                                raise ValueError(
                                    f"Failed to find exchange rate for {date}: {e}"
                                )
                        else:
                            current_usd_rate = float(
                                self.exchange_rates.loc[date, "USD"]
                            )
                        ...
                        if c_from == "USD" and c_to == "CAD":
                            ...
                            raise ValueError(
                                f"Unsupported conversion pair without explicit rate: {c_from}->{c_to}"
                            )

                    logger.info(
                        f"Applying conversion on {date.strftime('%Y-%m-%d')}: {amount:.2f} {c_from} -> {c_to} at {rate:.6g}"
                    )
                    ...
                    if c_from == "CAD" and c_to == "USD":
                        ...
                        raise ValueError(
                            f"Unsupported currency conversion: {c_from}->{c_to}"
                        )
                    ...
                    total_cad = current_cad_cash + (
                        current_usd_cash * self.exchange_rates.loc[date, "USD"]
                    )
            ...
            if date in self.trades.index:
                ...
                logger.debug(
                    f"Starting balances - CAD: ${current_cad_cash:.2f}, USD: ${current_usd_cash:.2f}"
                )
                ...
                for _, row in rows_for_date.iterrows():
                    ...
                    logger.debug(
                        f"Trade: {ticker} - {quantity} shares @ ${price:.2f} {currency}"
                    )
                    logger.debug(f"Trade value: ${trade_value:.2f} {currency}")

                    if quantity > 0:
                        # Buy: Deduct cash using helper
                        current_cad_cash, current_usd_cash, conversion_type = (
                            self._convert_currency_for_trade(
                                trade_value,
                                currency,
                                current_cad_cash,
                                current_usd_cash,
                        ...
                        if currency == "CAD":
                            ...
                            logger.debug(
                                f"Sell: Increased CAD cash by ${trade_value:.2f}"
                            )
                        elif currency == "USD":
                            current_usd_cash += trade_value
                            logger.debug(
                                f"Sell: Increased USD cash by ${trade_value:.2f}"
                            )
                        else:
                            # If unknown currency, default to CAD
                            current_cad_cash += trade_value
                            logger.debug(
                                f"Sell: Unknown currency, increased CAD cash by ${trade_value:.2f}"
                            )
                    ...
                    self.cash.at[date, "Total_CAD"] = total_cad
                    logger.debug(
                        f"After trade - CAD: ${current_cad_cash:.2f}, USD: ${current_usd_cash:.2f}, Total CAD: ${total_cad:.2f}"
                    )

            # Process dividends for this date
            if self.dividend_income is not None and date in self.dividend_income.index:
                logger.info(
                    f"--- Processing dividends on {date.strftime('%Y-%m-%d')} ---"
                )
                logger.debug(
                    f"Starting balances - CAD: ${current_cad_cash:.2f}, USD: ${current_usd_cash:.2f}"
                )
                ...
                for ticker in self.tickers:
                    dividend_amount = (
                        self.dividend_income.at[date, ticker]
                        if ticker in self.dividend_income.columns
                        else 0.0
                    )
                    ...
                    logger.debug(
                        f"Dividend: {ticker} - ${dividend_amount:.2f} {currency}"
                    )
                    ...
                    if currency == "CAD":
                        ...
                        current_cad_cash += dividend_amount
                        logger.debug(
                            f"Unknown currency, added ${dividend_amount:.2f} to CAD cash"
                        )
                    # Update cash balances
                    self.cash.at[date, "CAD_Cash"] = current_cad_cash
                    self.cash.at[date, "USD_Cash"] = current_usd_cash
                    ...
                    self.cash.at[date, "Total_CAD"] = total_cad
                    logger.debug(
                        f"After dividend - CAD: ${current_cad_cash:.2f}, USD: ${current_usd_cash:.2f}, Total CAD: ${total_cad:.2f}"
                    )

        pd.DataFrame(self.cash).to_csv(
            os.path.join(self.output_folder, cash_file), index_label="Date"
        )

    def _convert_currency_for_trade(
        self, trade_value, trade_currency, current_cad_cash, current_usd_cash, date
    ):
        """
        ...
        if trade_currency == "CAD":
            ...
            if current_cad_cash >= trade_value:
                ...
                logger.debug(
                    f"CAD trade - using existing CAD cash (${current_cad_cash:.2f} available)"
                )
                ...
                logger.debug(
                    f"Converting ${usd_to_convert:.2f} USD to CAD (rate: {exchange_rate:.4f})"
                )
            ...
            if current_usd_cash >= trade_value:
                ...
                logger.debug(
                    f"USD trade - using existing USD cash (${current_usd_cash:.2f} available)"
                )
                ...
                logger.debug(
                    f"  Converting ${cad_to_convert:.2f} CAD to USD (rate: {exchange_rate:.4f})"
                )
    ...
    def create_table_market_values(self):
        ...
        pd.DataFrame(self.market_values).to_csv(
            os.path.join(self.output_folder, market_values_file), index_label="Date"
        )
    ...
    def create_table_holdings(self):
        ...
        if self.trades is not None and not self.trades.empty:
            ...
            sorted_trades["split_factor"] = sorted_trades.apply(
                lambda r: cumulative_split_factor(r["Ticker"], r.name, latest_date),
                axis=1,
            )

            # Adjust Quantity and Price to be in "Today's Terms"
            sorted_trades["Quantity"] = (
                sorted_trades["Quantity"] * sorted_trades["split_factor"]
            )
            sorted_trades["Price"] = (
                sorted_trades["Price"] / sorted_trades["split_factor"]
            )
        ...
        for ticker, data in positions.items():
            ...
            total_return_native = (
                data["realized_pnl"] + unrealized_pnl + data["total_dividends"]
            )

            # ROI % (Native Return / Native Investment)
            # Math is identical regardless of currency
            return_pct = (
                (total_return_native / roi_denominator * 100.0)
            ...
            if first_purchase is None or roi_denominator <= 0:
                ...
                end_date = (
                    latest_date
                    if is_open
                    else last_sale_dates.get(ticker, first_purchase)
                )
                ...
                if days_held > 0:
                    annualized_return_pct = (
                        (1.0 + return_pct / 100.0) ** (365.0 / days_held) - 1.0
                    ) * 100.0
                else:
                    annualized_return_pct = float("nan")
            ...
            results.append(
                {
                    ...
                    "shares": qty,
                    "currency": currency,
                    "first_purchase_date": pd.Timestamp(first_purchase).date()
                    if first_purchase
                    else None,
                    ...
                    if qty > 0
                    else 0.0,  # Only show book value if open
                    "dividends": data["total_dividends"],
                    "realized_pnl": data["realized_pnl"],
                    "unrealized_pnl": unrealized_pnl,
                    "total_return": total_return_native,
                    "total_return_cad_normalized": total_return_native
                    * latest_fx_usd_cad,
        ...
        df["holding_weight"] = df["mv_cad_normalized"].apply(
            lambda x: (
                (x / total_portfolio_cad * 100.0) if total_portfolio_cad > 0 else 0.0
            )
        )
    ...
    def _build_currency_holdings(self):
        ...
        if self.holdings is not None and not self.holdings.empty:
            ...
            self.holdings_cad = (
                self.holdings[cad_cols]
                if len(cad_cols) > 0
                else pd.DataFrame(index=self.valid_dates)
            )
            self.holdings_usd = (
                self.holdings[usd_cols]
                if len(usd_cols) > 0
                else pd.DataFrame(index=self.valid_dates)
            )
        else:
    ...
    def _ensure_ticker_currency_map(self):
        ...
        """
        if self.ticker_currency_map and all(
            t in self.ticker_currency_map for t in (self.tickers or [])
        ):
            return
        ticker_currency_map = {}
        for ticker in self.tickers or []:
            try:
                ticker_currency_map[ticker] = yf.Ticker(ticker).info.get(
                    "currency", "CAD"
                )
            except Exception:
    ...
    def create_table_dividend_per_share(self):
        ...
        for ticker in self.tickers:
            ...
            self.dividend_per_share[ticker] = self.dividend_per_share.index.map(
                lambda x: divs.get(x, 0.0)
            )
        # Only keep rows with at least one nonzero value
        nonzero_div_per_share = self.dividend_per_share[
            (self.dividend_per_share != 0).any(axis=1)
        ]
        nonzero_div_per_share.to_csv(
            os.path.join(self.output_folder, dividend_per_share_file),
            index_label="Date",
        )
    ...
    def create_table_dividend_income(self):
        ...
        # Only keep rows with at least one nonzero value
        nonzero_div_income = self.dividend_income[
            (self.dividend_income != 0).any(axis=1)
        ]
        nonzero_div_income.to_csv(
            os.path.join(self.output_folder, dividend_income_file), index_label="Date"
        )
    ...
    def create_table_portfolio_total(self):
        ...
        cad_holdings_mv = (
            self.market_values[cad_cols].sum(axis=1)
            if len(cad_cols) > 0
            else pd.Series(0.0, index=self.valid_dates)
        )
        usd_holdings_mv = (
            self.market_values[usd_cols].sum(axis=1)
            if len(usd_cols) > 0
            else pd.Series(0.0, index=self.valid_dates)
        )
        ...
        pd.DataFrame(self.portfolio_total).to_csv(
            os.path.join(self.output_folder, portfolio_total_file), index_label="Date"
        )
    ...
    def print_final_values(self):
        ...
        logger.info(
            f"End Date: {(pd.to_datetime(self.end_date) - timedelta(days=1)).strftime('%Y-%m-%d')}"
        )
        logger.info("")
        logger.info(f"Starting Cash: {STARTING_CASH:.2f}")
        logger.info("")
        logger.info(
            f"CAD Cash (including dividends): {self.cash.loc[self.valid_dates[-1], 'CAD_Cash']:.2f}"
        )
        logger.info(
            f"USD Cash (including dividends): {self.cash.loc[self.valid_dates[-1], 'USD_Cash']:.2f}"
        )
        ...
        logger.info(
            f"Total Return: {((market_values_total + cash_total_cad) / STARTING_CASH - 1) * 100:.2f}%"
        )

...
def render_portfolio_summary(
    summary_data: Dict[str, Any], total_portfolio_value: float
):
    """
    ...
    with col3:
        ...
        st.metric(
            "Return Since Inception",
            inception_display,
            help="Cumulative return since inception (total return including dividends/cash)",
        )
...
def render_portfolio_breakdown(
    summary_data: Dict[str, Any],
    total_portfolio_value: float,
    cash_data: Dict[str, float],
):
    """
    ...
    with breakdown_col1:
        holdings_weight = (
            (holdings_value / total_portfolio_value * 100)
    ...
    with breakdown_col2:
        cash_weight = (
            (cash_data["Total_CAD"] / total_portfolio_value * 100)
            if total_portfolio_value > 0
            else 0
        )
        st.metric(

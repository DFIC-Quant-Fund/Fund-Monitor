...
def render_allocation_charts(holdings_data: pd.DataFrame):
    st.header("Portfolio Allocation (Equities)")
    if not holdings_data.empty:

        # Filter out Fixed Income from equity allocation calculations
        equity_only = holdings_data[holdings_data.get('sector', '') != 'Fixed Income'].copy()
        
        if equity_only.empty:
            st.info("No equity holdings found (all holdings are fixed income or cash)")
            return
        
        equity_value = equity_only['market_value'].sum()
        if equity_value <= 0:
            st.warning("Total equity market value is zero or negative")
            return
            
        equity_only['equity_weight_percent'] = (equity_only['market_value'] / equity_value * 100) if equity_value > 0 else 0
        
        col1, col2 = st.columns(2)
        with col1:
            sector_data = equity_only.groupby('sector')['equity_weight_percent'].sum().reset_index()
            ...
            fund_data = equity_only.groupby('fund')['equity_weight_percent'].sum().reset_index()
        ...
        with col2:
            geo_data = equity_only.groupby('geography')['equity_weight_percent'].sum().reset_index()
            ...
            st.info("💡 **Note**: Allocation charts show equity holdings only (excluding fixed income and cash).")
            render_allocation_summary(equity_only)
    else:

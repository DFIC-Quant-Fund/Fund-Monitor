...
def render_risk_metrics(risk_data: Dict[str, Any]):
    ...
    with col1:
        ...
        if "annualized_volatility" in risk_data:
            st.metric(
                "Annualized Volatility", f"{risk_data['annualized_volatility']:.2%}"
            )
    with col2:
        ...
        if "daily_downside_volatility" in risk_data:
            st.metric(
                "Downside Volatility", f"{risk_data['daily_downside_volatility']:.2%}"
            )
    with col3:
        if "annualized_downside_volatility" in risk_data:
            st.metric(
                "Annualized Downside Vol",
                f"{risk_data['annualized_downside_volatility']:.2%}",
            )
    st.markdown("---")
...
def render_risk_ratios(
    ...
    with col3:
        if "annualized_information_ratio" in ratios_data:
            st.metric(
                "Information Ratio",
                f"{ratios_data['annualized_information_ratio']:.3f}",
            )
    st.markdown("---")
...
def render_performance_summary(performance_data: Dict[str, Any]):
    ...
    if "risk_metrics" in performance_data:
        ...
        if "annualized_volatility" in risk:
            highlights.append(
                f"**Annualized Volatility:** {risk['annualized_volatility']:.2f}%"
            )
    ...
    if "ratios" in performance_data:
        ...
        if "annualized_sharpe_ratio" in ratios:
            highlights.append(
                f"**Sharpe Ratio:** {ratios['annualized_sharpe_ratio']:.3f}"
            )

...
class SecuritiesConfig:
                ...
                for security in config["securities"]:
                    if isinstance(security, dict) and "ticker" in security:
                        try:
                            ...
                            logger.exception(
                                f"Could not parse security {security.get('ticker', 'unknown')}: {e}"
                            )
    ...
    def get_securities_by_sector(
        self, sector: AuthoritativeSector
    ) -> List[SecurityInfo]:
        """Get all securities for a specific sector"""
        return [
            info for info in self._securities_data.values() if info.sector == sector
        ]

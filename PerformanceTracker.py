...
def total_return():
    ...
    total_return = (df['Total_Portfolio_Value'].iloc[-1] - df['Total_Portfolio_Value'].iloc[0]) / \
                   df['Total_Portfolio_Value'].iloc[0]
...
def main():
    ...
    print(
        f"Risk Adjusted Return (three month treasury rate),{risk_adjusted_return(THREE_MTH_TREASURY_RATE) * 100:.2f}%\n")
    ...
    with open(os.path.join(output_folder, 'performance_metrics.csv'), 'w') as f:
        ...
        f.write(
            f"SPY Benchmark Average Return (Annualized),{benchmark_average_return(benchmark='SPY')[1] * 100:.2f}%\n")
